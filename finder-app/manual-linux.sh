...
# changed kernel version to avoid compiler version error
#KERNEL_VERSION=v5.10.188
...
if [ ! -e ${OUTDIR}/linux-stable/arch/${ARCH}/boot/Image ]; then
    ...
    make -j 4 ARCH=arm64 CROSS_COMPILE=aarch64-none-linux-gnu- mrproper
    make -j 4 ARCH=arm64 CROSS_COMPILE=aarch64-none-linux-gnu- defconfig
    make -j 4 ARCH=arm64 CROSS_COMPILE=aarch64-none-linux-gnu- all
    make -j 4 ARCH=arm64 CROSS_COMPILE=aarch64-none-linux-gnu- modules
    make -j 4 ARCH=arm64 CROSS_COMPILE=aarch64-none-linux-gnu- dtbs
fi
